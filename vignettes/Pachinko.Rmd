...
```{r income_AWOTE_by_age_percentile}
...
```{r prob_5year_on_year_percentile}
...
An example of a the tensor product smooth applies to some dummy data is in the following figure. 
For the model, we used \(k = 10\).

```{r cache_invalidate, echo=FALSE}
...
```{r tensor-product-example, echo=FALSE, fig.width=13, fig.height=13}
...
<!-- Here is another example, where everyone in the sample is observed to stay in the same percentile between ages 25-29 and 30-34. This example shows that there is some preservation of -->

<!-- ```{r tensor-smooth-possibly-wrong} -->
<!-- dat <- CJ(x = seq(1, 100, by = 1), -->
<!--           y = seq(1, 100, by = 1)) -->
<!-- dat[, z := 0] -->
<!-- dat[x == y, z := 1] -->
<!-- dat[, predicted05 := predict(gam(z ~ te(x, y, k = c(5, 5)), data = dat), newdata = dat)] -->
<!-- dat[, predicted10 := predict(gam(z ~ te(x, y, k = c(10, 10)), data = dat), newdata = dat)] -->
<!-- dat[, predicted20 := predict(gam(z ~ te(x, y, k = c(20, 20)), data = dat), newdata = dat)] -->

<!-- dat_mat <-  -->
<!--   dcast.data.table(dat, y ~ x, value.var = "z") -->

<!-- # Enhance: hughparsonage/smoothie -->
<!-- # dat_smoothed <- kernel2dsmooth(as.matrix(drop_col(copy(dat_mat), "y")), kernel.type = "Cauchy", sigma = 2, nx = 100, ny = 100) -->

<!-- dat %>% -->
<!--   melt.data.table(measure.vars = c("z", "predicted05", "predicted10", "predicted20")) %>% -->
<!--   .[, value := value / sum(value), keyby = c("variable", "x")] %>% -->
<!--   .[, value := (value - min(value)) / max(value - min(value)), keyby = "variable"] %>% -->
<!--   .[variable != "z", variable := paste("k", "=", sub("predicted", "", variable))] %>% -->
<!--   ggplot(aes(x, y, fill = value)) + -->
<!--   geom_tile() +  -->
...
```{r lifetime-percentiels-for-initial-95percentile, fig.height=13}
